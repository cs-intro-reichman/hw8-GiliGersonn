...
public class Network {
   ...
   if (name1 == null || name2 == null) {
    ...
    if (name1.equals(name2)) {   // לא מאפשרים לעקוב אחרי עצמך
        return false;
    }
    ...
    for (int i = 0; i < userCount; i++) {
        ...
        if (mutual > bestMutual) {
            bestMutual = mutual;
            mostRecommendedUserToFollow = u2.getName();
        }
    }
    ...
    for (int i = 0; i < userCount; i++) {          // עוברים על כל המשתמשים
        User current = users[i];

        for (int j = 0; j < current.getfCount(); j++) {  // עוברים על ה-follows שלהם
            if (current.getfFollows()[j].equals(name)) {
                count++;
            }
        }
    }
